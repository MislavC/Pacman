...
public class Pacman extends Application {
   ...
   private VBox rootClient;// root for client
   ...
   private Label lblLives;// label for lives
   private Label lblPoints;// label for points
   ...
   public void startMenu() {// starting menu
      ...
      btnAbout.setAlignment(Pos.BOTTOM_CENTER);
      btnExit = new Button("Exit");// exit button
      btnChat = new Button("Chat");
      btnChat.setAlignment(Pos.BOTTOM_CENTER);
      btnExit.setAlignment(Pos.BOTTOM_CENTER);
      fp1.setAlignment(Pos.BOTTOM_CENTER);
      fp1.getChildren().addAll(btnIn, btnExit, btnChat, btnAbout);
      ...
      // btnChat.setOnAction(e ->client.start(clientStage));
      btnChat.setOnAction(e -> clients.start(clientStage));
   ...
   public void about() {// about screen
      VBox root5 = new VBox();
      Scene scene5 = new Scene(root5, 800, 500);// screen dimensions
      scene5.getStylesheets().addAll(this.getClass().getResource("style.css").toExternalForm());

      FlowPane fp5 = new FlowPane(8, 8);
      Button btnBack = new Button("Go Back!");// go back
      ...
      fp5.getChildren().addAll(taAbout, btnBack);
      root5.getChildren().addAll(fp5);
      root5.setAlignment(Pos.CENTER);
      root5.setId("about");// css to get image
      stage.setScene(scene4);
      stage.show();

      btnBack.setOnAction(e -> startMenu());// going to start menu
   ...
   protected class PacmanRacer extends Pane {
      ...
      public void update() {
         ...
         readerRight = img.getPixelReader().getColor(this.racePosX + 60, this.racePosY + 30);// using pixel reader for
                                                                                             // right side
         readerLeft = img.getPixelReader().getColor(this.racePosX + 60, this.racePosY + 30);// using pixel reader for
                                                                                            // left side
         readerUp = img.getPixelReader().getColor(this.racePosX + 30, this.racePosY + 60);// using pixel reader for up
         readerDown = img.getPixelReader().getColor(this.racePosX + 30, this.racePosY + 60);// using pixel reader for
                                                                                            // down
         ...
         for (int i = 0; i < coins.size(); i++) {
            int position = (int) Math.sqrt(Math.pow(racer.getRacePosX() - coins.get(i).getPosX(), 2)
                  + Math.pow(racer.getRacePosY() - coins.get(i).getPosY(), 2));// getting the position of the coin with
                                                                               // racer positons and math equations
            if (position < 15) {
         ...
         for (int i = 0; i < fruit.size(); i++) {
            int position = (int) Math.sqrt(Math.pow(racer.getRacePosX() - fruit.get(i).getPosX(), 2)
                  + Math.pow(racer.getRacePosY() - fruit.get(i).getPosY(), 2));// getting the position of the fruti with
                                                                               // racer positons and math equations
            if (position < 15) {
         ...
         for (int i = 0; i < ghost.size(); i++) {
            int position = (int) Math.sqrt(Math.pow(racer.getRacePosX() - ghost.get(i).getPosX(), 2)
                  + Math.pow(racer.getRacePosY() - ghost.get(i).getPosY(), 2));// getting the position of the ghost with
                                                                               // racer positons and math equations
            if (position < 15) {
